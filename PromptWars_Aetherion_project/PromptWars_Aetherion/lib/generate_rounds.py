from PromptWars_Aetherion.lib.types import Round, PromptPart


def generate_rounds() -> list[Round]:
    return [
        # ── Round 1: CLASSIFY ──────────────────────────────────────────
        Round(
            roundNumber=1,
        ...
        Round(
            roundNumber=2,
            type="IMPROVE",
            instruction="The original prompt below was given to the AI, but it did not produce an ideal output. Your task is to improve this prompt to produce the desired structured output. Improve the given prompt to extract structured highlights (Conflicts, Decisions, Dependencies, Next Steps) in ≤90 words.",
...
Assign ownership for risks, QA, and supply chain mitigation
      """,
            constraints={
                "maxWords": 90,
                "requiredSections": ["Conflicts", "Decisions", "Dependencies", "Next Steps"],
            },
        ),
...
User drop-off after initial engagement""",
            constraints={
                "requiredSections": ["Problem", "Solution", "Audience", "Features", "Revenue", "Differentiator", "Tagline", "Risks"],
            },
        ),

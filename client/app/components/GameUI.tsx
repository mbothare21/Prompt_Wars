...
export default function GameUI() {
  ...
  const submitPrompt = async (overrideAnswers?: Record<string, string>) => {
    ...
    try {
      ...
      if (status === "GAME_COMPLETED") {
        ...
        // Freeze the displayed Duration now so it matches what we just
        // persisted to the DB — the timer would otherwise keep ticking during
        // the 15s bonus result modal and the phase transition, drifting the
        // Game Over screen ahead of the stored timeTaken.
        if (gameEndedSecondsUsedRef.current === null) {
          const timeTakenMs = typeof data.timeTakenMs === "number" ? data.timeTakenMs : null;
          if (timeTakenMs !== null) {
            gameEndedSecondsUsedRef.current = Math.max(0, Math.round(timeTakenMs / 1000));
          }
        }
  ...
  // The report HTML is built entirely server-side (server/lib/playerReport.ts)
  // so the ideal-prompt content never appears in any client-visible payload.
  // These helpers just stream the response into a download.
  const downloadHtmlResponse = async (res: Response, fallbackName: string) => {
  ...
  const downloadPlayerResponses = async (email: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        let message = "Failed to fetch player data";
        try {
      ...
      await downloadHtmlResponse(res, "player-report.html");
    } catch {
  ...
  const downloadMyReport = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        let message = "Failed to fetch your report";
        try {
          const data = (await res.json()) as { error?: string };
          if (data.error) message = data.error;
        } catch {
      ...
      await downloadHtmlResponse(res, "my-prompt-wars-report.html");
    } catch {
